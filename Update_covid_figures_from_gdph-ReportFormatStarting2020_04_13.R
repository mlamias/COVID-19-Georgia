...
race_table <-
  html %>%  html_nodes(xpath = '//*[@id="race2"]/table') %>%  simplify() %>%  pluck(1) %>% html_table(header=TRUE)
...
#COVID_19_GEORIGA_DATA %>% mutate(race_black_pct=0.00, race_white_pct=0.00, race_other_pct=0.00, race_unknown_pct=0.00) %>% 
 # select(1:sex_unknown_pct, race_black_pct:race_unknown_pct, everything()) ->COVID_19_GEORIGA_DATA
...
saveRDS(
  COVID_19_GEORIGA_RACE_ETHNICITY_DATA_CURRENT,
  file = paste0(DATA_DIRECTORY, "/COVID_19_GEORIGA_RACE_ETHNICITY_DATA.Rds")
)
...
write.csv(
  COVID_19_GEORIGA_RACE_ETHNICITY_DATA_CURRENT,
  file = paste0(DATA_DIRECTORY, "/COVID_19_GEORIGA_RACE_ETHNICITY_DATA.csv"),
  row.names = FALSE
)
